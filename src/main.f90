   ...
   write(*,*)
   write(*,*),'                    DSurfRTomo' 
   write(*,*),'PLEASE contact Shaoqian Hu &
              (husq@ustc.edu.cn) if you find any bug' 
   write(*,*)

   ! read input file
   if (iargc()<1) then
      write(*,*) 'input file [DSurfRTomo.in(default)]'
      read(*,'(a)') inputfile
      if (len_trim(inputfile)<=1) then
         inputfile='DSurfRTomo.in'
      else
   ...
   do iter=1,maxiter
      ...
      write(*,'(a,2f7.3)'), 'weight1 and weight2 are: ', weight1, weight2
      ...
      write(*,'(a,2f7.4)'),'min and max gamma variation ',&
